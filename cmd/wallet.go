...
var (
	...
	walletImportCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			var kindRaw string
            if argEd25519Priv != "" {
                kindRaw = string(wallet.ImportKindPrivateKey)
            } else {
            ...
            var afCfg map[string]interface{}
            if argEd25519Priv != "" {
                afCfg = map[string]interface{}{
	...
	walletRemoteSignerCmd = &cobra.Command{
		...
		Short: "Act as a hela-node remote entity signer over AF_LOCAL",
		Args:  cobra.ExactArgs(2),
		Run: func(cmd *cobra.Command, args []string) {
