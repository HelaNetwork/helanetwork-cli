...
var (
	...
	accountsShowCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if npa.ParaTime != nil {
				...
				rtRole, err := c.Runtime(npa.ParaTime).Accounts.Role(ctx, round, *addr)
				cobra.CheckErr(err)
				rtInit, err := c.Runtime(npa.ParaTime).Accounts.InitInfo(ctx, round, *addr)
				cobra.CheckErr(err)

				fmt.Println()
				fmt.Printf("=== %s PARATIME ===\n", npa.ParaTimeName)
				fmt.Printf("Role is: %s; init status is: %t.\n", rtRole.String(), rtInit)
	...
	accountsDepositCmd = &cobra.Command{
		...
		Short: "Deposit given amount of tokens into an account in the runtime",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if npa.ParaTime == nil {
				cobra.CheckErr("no runtime to deposit into")
			}
            ...
            var ethToAddr [20]byte
			if to != "" {
				var err error
				toAddr, err = common.ResolveLocalAccountOrAddress(npa.Network, to)
				cobra.CheckErr(err)

                if _, addr, _ := helpers.ResolveEthOrOasisAddress(to); addr != nil {
                    for i, value := range addr {
                        ethToAddr[i] = value
                    }
                } else {
                    acc, err := cfg.Wallet.Load(to, "")
                    cobra.CheckErr(err)
                    if addr := acc.EthAddress(); addr != nil {
                        for i, value := range addr {
                            ethToAddr[i] = value
                        }
                    }
	...
	accountsWithdrawCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if npa.ParaTime == nil {
				cobra.CheckErr("no runtime to withdraw from")
			}
            ...
            var ethFromAddr [20]byte
            if addr := acc.EthAddress(); addr != nil {
                fmt.Printf("eth addr: %x\n", addr)
                for i, value := range addr {
                    ethFromAddr[i] = value
                }
            } else {
                fmt.Printf("account not support withdraw\n")
                return
            }
	...
	accountsMintSTCmd = &cobra.Command{
		Use:   "mintst <amount> <to>",
		Short: "Mint given amount of stable tokens (SUSD) to an account by NetworkInitiator",
		Args:  cobra.ExactArgs(2),
		Run: func(cmd *cobra.Command, args []string) {
			cfg := cliConfig.Global()
			npa := common.GetNPASelection(cfg)
			...
			switch npa.ParaTime {
			case nil:
				// GB: ignore currently as the mint only happens in paratime layer.
			...
			default:
				...
				// TODO: This should actually query the ParaTime (or config) to check what the consensus
				//       layer denomination is in the ParaTime. Assume NATIVE for now.
				// GBTODO: Merge with Mingzhe's part to configure the base unit.
				// GBDONE: Mint SUSD directly.
				amountBaseUnits, err := helpers.ParseParaTimeDenomination(npa.ParaTime, amount, types.NativeDenomination)

				cobra.CheckErr(err)

				// Prepare transaction.
				tx := accounts.NewMintSTTx(nil, &accounts.MintST{
					To:     *toAddr,
					Amount: *amountBaseUnits,
				})
	...
	// GB: insert BurnST (stable coin) function command
	accountsBurnSTCmd = &cobra.Command{
		Use:   "burnst <amount>",
		Short: "Burn given amount of stable tokens (SUSD)",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			switch npa.ParaTime {
			case nil:
				// GB: ignore currently as the mint/burn only happens in paratime layer.
				err := "Invalid paratime configured!"
				cobra.CheckErr(err)
			...
			default:
				...
				// Prepare transaction.
				tx := accounts.NewBurnSTTx(nil, &accounts.BurnST{
					// To:     *toAddr,
					Amount: *amountBaseUnits,
				})
	...
	// MZ
	accountsTransferStableCmd = &cobra.Command{
		Use:   "transferstable <amount> <to>",
		Short: "Transfer given amount of stable tokens (HLUSD) to a different account (currently only can work in hela-evm)",
		Args:  cobra.ExactArgs(2),
		Run: func(cmd *cobra.Command, args []string) {
			cfg := cliConfig.Global()
			npa := common.GetNPASelection(cfg)
	...
	// MZ
	// Temporarily used for faucet
	accountsFaucetTransferCmd = &cobra.Command{
		Use:   "fautransfer <amount> <to>",
		Short: "Temporarily used for faucet, transfer given amount of tokens to a different account",
		Args:  cobra.ExactArgs(2),
		Run: func(cmd *cobra.Command, args []string) {
			cfg := cliConfig.Global()
			npa := common.GetNPASelection(cfg)
			...
			case nil:
				...
				sigTx, err = common.SignConsensusTransactionFaucet(ctx, npa, acc, conn, tx)
				cobra.CheckErr(err)
			default:
				// ParaTime transfer.
				// TODO: This should actually query the ParaTime (or config) to check what the consensus
				...
				sigTx, meta, err = common.SignParaTimeTransactionFaucet(ctx, npa, acc, conn, tx)
				cobra.CheckErr(err)
			}
	...
	accountsBurnCmd = &cobra.Command{
		Use:   "burnct <amount>",
		Short: "Burn given amount of consensus tokens (HELA)",
		Args:  cobra.ExactArgs(2),
		Run: func(cmd *cobra.Command, args []string) {
			cfg := cliConfig.Global()
			npa := common.GetNPASelection(cfg)
			...
			// GBTO: Configure paratime here
			if npa.ParaTime != nil {
				cobra.CheckErr("burns within runtime are not supported; use --no-runtime")
			}
	...
	accountsDelegateCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			default:
				// ParaTime delegation.
				cobra.CheckErr("delegations within runtime are not supported; use --no-runtime")
			}
...
func init() {
	...
	// GB: insert here accountsMintSTCmd.
	accountsMintSTCmd.Flags().AddFlagSet(common.SelectorFlags)
	accountsMintSTCmd.Flags().AddFlagSet(common.TransactionFlags)
	accountsMintSTCmd.Flags().AddFlagSet(common.ForceFlag)

	// GB: insert here accountsBurnSTCmd.
	accountsBurnSTCmd.Flags().AddFlagSet(common.SelectorFlags)
	accountsBurnSTCmd.Flags().AddFlagSet(common.TransactionFlags)
	accountsBurnSTCmd.Flags().AddFlagSet(common.ForceFlag)

	// MZ
	accountsTransferStableCmd.Flags().AddFlagSet(common.SelectorFlags)
	accountsTransferStableCmd.Flags().AddFlagSet(common.TransactionFlags)
	accountsTransferStableCmd.Flags().AddFlagSet(common.ForceFlag)

	// MZ
	accountsFaucetTransferCmd.Flags().AddFlagSet(common.SelectorFlags)
	accountsFaucetTransferCmd.Flags().AddFlagSet(common.TransactionFlags)
	accountsFaucetTransferCmd.Flags().AddFlagSet(common.ForceFlag)
	...
	// GB: insert here accountsMintSTCmd & accountsBurnSTCmd.
	accountsCmd.AddCommand(accountsMintSTCmd)	
	accountsCmd.AddCommand(accountsBurnSTCmd)	


	// MZ
	accountsCmd.AddCommand(accountsTransferStableCmd)
	accountsCmd.AddCommand(accountsFaucetTransferCmd)
